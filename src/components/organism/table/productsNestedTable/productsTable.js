...
const ProductsTable = (props) => {
  ...
  const barcodeGenerator = (record) => {
    ...
    let redirectString = `http://tools.workify.xyz/barcode/index.php?barcode=${
      record.product_sku
    }
            &name=${record.product_sku}%20&count=${count}&price=${symbol}${
      record.product_sale_price
    }
            &logo=${storeObj && storeObj.store_name}&pname=${
      record.product_name
    }`;
