...
const ProductsVariantsTable = (props) => {
  ...
  const tableExpandedRowRender = (record, index, indent, expanded) => {
    ...
    return (
      <ProductsVariantsNestedTable
        tableData={outletsData}
        userStores={props.userStores}
        taxes={props.taxes}
  ...
  const handleSaveUpdatedNestedVariantsData = (
    updatedOutletsVariantsNestedData,
    currentExpandedRow
  ) => {
    var newData = [...data];
    const index = newData.findIndex(
      (item) => currentExpandedRow == item.variant_row_id
    );
    if (index > -1) {
  ...
  const handleSave = async (row) => {
    const newData = [...data];
    const index = newData.findIndex(
      (item) => row.variant_row_id === item.variant_row_id
    );
          ...
          <div>
            {
              <span>
                {record.var1_text && record.var2_text ? (
                  <small>{record.var1_text + "/ " + record.var2_text}</small>
                ) : record.var1_text ? (
                  <small>{record.var1_text}</small>
                ) : record.var2_text ? (
                  <small>{record.var2_text}</small>
                ) : (
                  ""
                )}
              </span>
  ...
  const mergedColumns = columns.map((col) => {
    ...
    return {
      ...
      onCell: (record) => ({
        ...
        inputType:
          col.dataIndex === "purchase" || col.dataIndex === "sale"
            ? "number"
            : "text",
