...
export const getRegisteredProducts = async (limit, PageNumber) => {
    ...
    const viewRegisteredProductsFormDataBody = ApiCallUtil.constructFormData(
        formDataPair
    );
    ...
    for (let i = 0; i < discountedProducts.length; i++) {
        Object.entries(discountedProducts[i]).forEach(
            ([formDataKey, formDataValue]) => {
                discountedProductsFormDataBody.append(
                    `products[${i}][${formDataKey}]`,
                    formDataValue
                );
            }
        );
...
export const searchProducts = async (limit, PageNumber, searchvalue) => {
    ...
    const searchProductsFormDataBody = ApiCallUtil.constructFormData(
        formDataPair
    );
    const url = UrlConstants.PRODUCTS.VIEW;
    const callType = GenericConstants.API_CALL_TYPE.POST;
